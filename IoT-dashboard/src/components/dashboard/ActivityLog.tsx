  ...
  return (
    <section className="card mt-4 sm:mt-6">
      <h2 className="flex items-center gap-2 sm:gap-[10px] text-[1rem] sm:text-[1.1rem] font-bold mb-4 sm:mb-5">
        <History size={18} className="sm:size-5 text-slate-400 shrink-0" />
      ...
      <div
        className="max-h-[180px] sm:max-h-[220px] overflow-y-auto"
        style={{ scrollbarWidth: "thin", scrollbarColor: "#cbd5e1 transparent" }}
      >
        {logs.length === 0 ? (
          <div className="text-center text-slate-400 py-6 text-[0.875rem]">
            No activity yet
          </div>
        ) : (
          logs.map((entry) => (
            <div
              key={entry.id}
              className="flex items-center gap-2 sm:gap-3 py-2 sm:py-[10px] border-b border-slate-100 dark:border-slate-700 last:border-b-0 text-[0.8125rem] sm:text-[0.875rem]"
            >
              <span className={`w-2 h-2 rounded-full shrink-0 ${dotClass[entry.type] ?? "bg-slate-400"}`} />
