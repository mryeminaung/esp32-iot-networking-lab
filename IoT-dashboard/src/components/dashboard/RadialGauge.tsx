...
export default function RadialGauge({ value, size = 160 }: RadialGaugeProps) {
  ...
  const gaugeSize = size > 120 ? "w-[130px] h-[130px] sm:w-[160px] sm:h-[160px]" : "w-full"

  /* Clamp & compute offset */
  const clamped = Math.max(0, Math.min(100, value))
  const offset = circumference - (clamped / 100) * circumference

  /* Determine color: green > 70%, yellow > 40%, red below */
  const color =
  ...
  return (
    <div className={`relative mx-auto ${gaugeSize}`} style={{ aspectRatio: "1" }}>
      <svg
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center leading-tight">
        ...
        <span className="text-[0.75rem] sm:text-[0.875rem] font-semibold text-slate-500">%</span>
